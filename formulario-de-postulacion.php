	  ...
	  $SQLconsultaPostulacion = " SELECT * FROM postulaciones AS p WHERE p.id_usuario = '$id_usuario' AND p.id_programa = '$id_programa' ";
		...
		if ($existePostulacion > 0)
		{
		  ...
		  $registro_RegPostulacion = $SQLconsultaRegPostulacion->fetch_assoc();
				...
				$fincedula = $registro_RegPostulacion['fincedula']; //36
				$finroldepago = $registro_RegPostulacion['finroldepago']; //37
				$finplanillaserbasico = $registro_RegPostulacion['finplanillaserbasico']; //38
				$finmatriculacarro = $registro_RegPostulacion['finmatriculacarro']; //39
				$finprediocasa = $registro_RegPostulacion['finprediocasa']; //40
				$finautorburocredito = $registro_RegPostulacion['finautorburocredito']; //41
				$fincartaceptacion = $registro_RegPostulacion['fincartaceptacion']; //42
				$fincalificacion = $registro_RegPostulacion['fincalificacion']; //43

				$fecha_postulacion = $registro_RegPostulacion['fecha_postulacion']; // 44
				$modificado_por = $registro_RegPostulacion['modificado_por']; // 45
				$fecha_ult_modificacion = $registro_RegPostulacion['fecha_ult_modificacion']; // 46
				$version_form = $registro_RegPostulacion['version_form']; // 47
			...
			else
		{
		  ...
		  $registro_DatosUsuario = $SQLconsultaDatosUsuario->fetch_assoc();
				...
				$fincedula = $registro_DatosUsuario['fincedula']; //36
				$finroldepago = $registro_DatosUsuario['finroldepago']; //37
				$finplanillaserbasico = $registro_DatosUsuario['finplanillaserbasico']; //38
				$finmatriculacarro = $registro_DatosUsuario['finmatriculacarro']; //39
				$finprediocasa = $registro_DatosUsuario['finprediocasa']; //40
				$finautorburocredito = $registro_DatosUsuario['finautorburocredito']; //41
				$fincartaceptacion = $registro_DatosUsuario['fincartaceptacion']; //42
				$fincalificacion = $registro_DatosUsuario['fincalificacion']; //43

				$fecha_postulacion = $registro_DatosUsuario['fecha_postulacion']; // 44
				$modificado_por = $registro_DatosUsuario['modificado_por']; // 45
				$fecha_ult_modificacion = $registro_DatosUsuario['fecha_ult_modificacion']; // 46
				$version_form = $registro_DatosUsuario['version_form']; // 47
		...
		*/
		...
		switch ($version_form) {
	    case 1:
	      $archivo_version = "formulario-doctorado-archivo-version-uno.php";
	      break;
	    case 2:
	      $archivo_version = "formulario-doctorado-archivo-version-dos.php";
	      break;
	    default:
	    	$archivo_version = "formulario-doctorado-archivo-version-dos.php"; //En caso que en la BD no este definido la version, carga la version 2 por defecto
		}
