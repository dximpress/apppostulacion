...
$SQLProspectos = " 
	SELECT
		...
		UPPER(pr.nombre) AS programa,
		p.correo AS correo,
		p.tcelular AS tcelular,
		p.provincia AS provincia,
		p.ciudad AS ciudad,
		f.aprobadofinrep AS estadofinanciamiento,
		IF(f.aprobadofinrep IS NULL, 'Por revisar', e.valor) AS estado,
	    IF(f.f_aplicafinanciamiento IS NULL, '---', DATE_FORMAT(f.f_aplicafinanciamiento, '%d - %m - %Y %H:%i:%s')) AS f_i_solicitud,
	    CASE
	    	WHEN f.f_aplicafinanciamiento IS NULL THEN '---'
			WHEN DAYOFWEEK(f.f_aplicafinanciamiento) = '5' THEN DATE_FORMAT(DATE_ADD(f.f_aplicafinanciamiento, INTERVAL 3 DAY), '%d - %m - %Y %H:%i:%s') #Viernes
	        WHEN DAYOFWEEK(f.f_aplicafinanciamiento) = '6' THEN DATE_FORMAT(DATE_ADD(f.f_aplicafinanciamiento, INTERVAL 2 DAY), '%d - %m - %Y %H:%i:%s') #Sabado
	        ELSE DATE_FORMAT(DATE_ADD(f.f_aplicafinanciamiento, INTERVAL 1 DAY), '%d - %m - %Y %H:%i:%s') #Lunes, Martes, Miercoles, Jueves, Domingo
	    END f_f_solicitud,
		f.fincalificacion AS estadofinanciamientocop,
		IF(f.fincalificacion IS NULL, 'Por revisar', ecop.valor) AS estadocop,
	...
	LEFT JOIN estado AS e ON f.aprobadofinrep = e.id
	LEFT JOIN estado AS ecop ON f.fincalificacion = ecop.id
	LEFT JOIN programas AS pr ON pr.id = f.id_programa
	CROSS JOIN (SELECT @cnt:=0) AS dummy
	WHERE aplicafinanciamiento = 1
";
